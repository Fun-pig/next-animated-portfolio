...
export default function AboutPage() {
  ...
  // const isSkillRefInView = useInView(skillRef, { once: true }); // 判断是否在视口内
  const isSkillRefInView = useInView(skillRef, { margin: "-100px" }); // 判断是否在视口内

  const experienceRef = useRef();
  // const isExperienceRefInView = useInView(experienceRef, { once: true }); // 判断是否在视口内
  const isExperienceRefInView = useInView(experienceRef, { margin: "-100px" }); // 判断是否在视口内
  return (
    <motion.div
      className="h-full"
      initial={{ y: "-200vh" }}
